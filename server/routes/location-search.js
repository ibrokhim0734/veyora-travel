...
module.exports=async(req,res)=>{try{const q=String((req.query||{}).q||'').trim(),kind=String((req.query||{}).kind||'all').toLowerCase();if(q.length<2)return res.status(200).json({mode:'local',results:[]});let results=[],mode='live';try{const sub=kind==='airport'?'AIRPORT':kind==='city'?'CITY':'CITY,AIRPORT';const d=await amadeus(`/v1/reference-data/locations?subType=${encodeURIComponent(sub)}&keyword=${encodeURIComponent(q)}&view=LIGHT`);if(!d)throw new Error('not configured');results=(d.data||[]).slice(0,10).map(x=>({code:x.iataCode||x.address?.cityCode||'',name:x.name||x.address?.cityName||'',detail:[x.subType,x.address?.cityName,x.address?.countryName].filter(Boolean).join(' • '),type:x.subType||'',country:x.address?.countryCode||''})).filter(x=>x.code)}catch{mode='local'}if(!results.length){const s=q.toLowerCase();results=fallback.filter(x=>(kind==='all'||kind==='airport'&&x.type==='AIRPORT'||kind==='city'&&x.type==='CITY')&&(`${x.code} ${x.name} ${x.detail}`.toLowerCase().includes(s))).slice(0,10)}res.status(200).json({mode,results})}catch(e){res.status(500).json({error:e.message||'Location search failed'})}};